...
namespace GameCore
{
    public class MCTS : Singleton<MCTS>
    {
        private class Node
        {
            ...
            public Decision.Message decisionMessage;
            ...
            switch (state)
            {
                ...
                case State.WaitTimer:
                    ...
                    foreach (var i in Timer.Instance.multiConvert.Where(x => Timer.Instance.isValidCard(x)))
                    {
                        temp.cards.Add(i);
                        // temp.
                        dests = Main.Instance.AlivePlayers.Where(x => Timer.Instance.isValidDest(x)).ToList();
        ...
        private async Task Simulate(Decision.Message decision)
        {
        ...
        {
            float sum = 0;
            foreach (var i in Main.Instance.AlivePlayers)
            {
        ...
        private void Traverse2Dests()
        {
            ...
            {
                temp.dests.Add(i);
                var secondDests = Main.Instance.AlivePlayers.Where(x => Timer.Instance.isValidDest(x) && x != i);
