...
public class AnimationManager : SingletonMono<AnimationManager>
{
    ...
    /// 武将位置
    /// </summary>
    private Vector3 Pos(int playerIndex) => Game.Instance.players[playerIndex].transform.position;
    ...
    private void OnUseCard(UseCard useCard)
    {
        ...
        var asset = GameAsset.Instance.cardEffect.Get(cardName);
    ...
    private void OnDamage(Damage damage)
    {
        ...
        var prefab = type == Damage.Type.Thunder ? GameAsset.Instance.thunderDamageEffect
            : type == Damage.Type.Fire ? GameAsset.Instance.fireDamageEffect : GameAsset.Instance.normalDamageEffect;
