...
public class SkillArea : SingletonMono<SkillArea>
{
    ...
    private Player self => Game.Instance.firstPerson;
    ...
    public void InitOnePlayer(List<string> skills, Model.Player src)
    {
        ...
        bool l = false;
        // 实例化预制件，添加到技能区
        foreach (var i in skills.Select(x => SkillAsset.Get(x)))
        {
            GameObject prefab;
            switch (i.type)
            {
                case SkillAsset.Type.Limited: prefab = 限定技; break;
                case SkillAsset.Type.Passive: prefab = 锁定技; break;
                default: prefab = 主动技; break;
            }
            var skill = Instantiate(prefab).GetComponent<Skill>();

            skill.gameObject.SetActive(src == self.model);
            skill.Init(i.name, src);
