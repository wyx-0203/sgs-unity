...
public class Personal : MonoBehaviour
{
    ...
    {
        var msg = await WebRequest.Get($"{Url.DOMAIN_NAME}getUserInfo?id={Global.Instance.userId}");
        ...
        character.sprite = await SkinAsset.Get(Int32.Parse(json.character)).GetSeatImage();
    }
